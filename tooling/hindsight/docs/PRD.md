...
Hindsight deployments need one portable control-plane contract across machines
and agent harnesses. Profile, bank, provider, runtime, and harness concerns must
remain independently configurable, while bank missions, policies, provider
compatibility, and migration safety remain reproducible desired state.

Unsafe consumer rendering can erase useful live adapter settings, while an
embedding-model or bank-policy change can silently require a data migration.
Hindsight's native bank isolation also does not, by itself, enforce cross-bank
routing, prompt-injection boundaries, or a common policy across harnesses with
...
live state, credentials, and irreversible mutations outside ordinary consumer
configuration rendering or installation.

## Solution

Build one desired-state Hindsight control plane around a versioned machine
inventory. `nisavid/agents` owns reusable controller code, bank archetypes,
provider catalogs, harness policies, validation schemas, skills, tests, and
documentation. A consuming configuration repository owns machine inventory,
secret locators, service bindings, and inactive rendered harness artifacts. The
controller owns live reconciliation through `validate`, `plan`, `status`,
`apply`, and explicit prune and migration operations.
...
1. As the operator, I want one versioned inventory per machine, so that the deployed
   memory topology is reviewable and reproducible.
2. As the operator, I want to select a named deployment archetype with sparse
   overrides, so that common machines are easy to configure without hiding
   provider choices.
3. As the operator, I want hardware and credential probes to validate desired state
   without rewriting it, so that configuration remains deterministic.
4. As the operator, I want each runtime profile to declare providers, models, ports,
   authentication references, and lifecycle policy, so that independent
   runtimes cannot collide silently.
5. As the operator, I want one fleet supervisor to own the control service and all
...
8. As an operator, I want profile ports derived from persisted slots with
   ...
   validation.
9. As the operator, I want provider definitions reused by explicit role bindings, so
   that LLM, embedding, and reranking constraints are modeled separately.
10. As the operator, I want embedding model identity treated as migration-class state,
    so that a vector-space change cannot be applied in place.
11. As the operator, I want LLM failover to be declared independently from embedding
    identity, so that generation availability does not corrupt stored vectors.
12. As the operator, I want reranker fallback to be explicit and visible, so that a
    quality degradation never looks like the selected configuration.
13. As the operator, I want stable hosted API keys resolved from a platform
    credential store, so that credential values never enter configuration,
    templates, plans, or logs.
14. As the operator, I want OAuth state established interactively for one machine and
...
17. As the operator, I want each profile inventory to support zero or more engineering
...
18. As the operator, I want routine engineering recall to remain global, so that a
    seemingly unrelated prior lesson can still be useful.
19. As the operator, I want repository and workflow identity to enrich provenance and
    model selection without becoming routine recall barriers.
20. As the operator, I want a first-class optional personal bank, so that explicitly
...
27. As the operator, I want every automatic cross-bank action announced with source,
    ...
    accumulate.
28. As the operator, I want projections to be minimal, idempotent, provenance-linked,
...
30. As an agent user, I want clean task outcomes retained separately from the
    ...
    raw tool traffic.
31. As the operator, I want source, harness, canonical repository, controlled workflow,
    and lifecycle tags to be deterministic, so that recall and reconciliation
    do not depend on content classification.
32. As the operator, I want session, branch, worktree, and other volatile identifiers in
    metadata rather than tags, so that observation scopes do not fragment.
33. As the operator, I want each retain to contribute to exactly one semantic
    observation scope, so that consolidation cost and duplicate beliefs remain
    bounded.
34. As the operator, I want a canonical repository scope when repository identity is
    reliable and one global active scope otherwise, so that local patterns and
    cross-domain knowledge both have coherent homes.
35. As the operator, I want LLM-emitted knowledge kinds to support investigation and
...
37. As an agent user, I want routine responses to use recall plus direct mental
    ...
    prompt.
38. As the operator, I want `reflect` to remain explicit, cited, and audited, so that
...
44. As the operator, I want mental models refreshed on staggered, change-aware
    schedules with health and queue gates, so that freshness does not recreate
    the prior quota and operation backlog.
45. As the operator, I want mental-model refreshes to exclude mental models as source
    evidence, so that summaries cannot recursively launder themselves into
    facts.
46. As the operator, I want an explicit trust mode at session start and policy-forced
...
48. As the operator, I want airlock text stored as searchable chunks with extraction and
    observations disabled, so that hostile instructions are not synthesized
    into durable beliefs.
49. As the operator, I want an airlock to produce an encrypted audit export and
    source-cited bridge candidates, so that useful lessons can be reviewed
    without automatic promotion.
50. As the operator, I want a verified airlock bank deleted immediately after closeout,
...
55. As the operator, I want unknown harness-owned settings preserved during rendering,
    so that enabling managed memory does not erase unrelated configuration.
56. As the operator, I want rendered hook artifacts disabled until an approved
    controller activation, so that consumer configuration rendering cannot
    begin recall or retention.
57. As an operator, I want activation bound to an immutable digest and endpoint,
    ...
    become active.
58. As the operator, I want one `hindsight-memory session` entry point for home-bank and
    trust-mode selection, so that launch-time policy is consistent across CLI
    and GUI harnesses.
59. As the operator, I want Codex, Claude, and portable Markdown/JSONL import adapters,
    so that prior memory sources can be reconciled without one-off scripts.
60. As the operator, I want every import item assigned a stable identity, timestamp,
    provenance, and coverage disposition, so that imports are resumable,
    reviewable, and reversible.
61. As the operator, I want heuristic novelty and deduplication to propose rather than
    apply outcomes, so that imports remain digest-bound approval artifacts.
62. As the operator, I want the deferred raw Codex corpus mined only for novel durable
...
65. As the operator, I want model recommendations earned on a private, human-vetted
    local retrieval benchmark, so that model cards and hosted-teacher agreement
    do not become the quality authority.
66. As the operator, I want model promotion to require no material retrieval regression,
...
67. As the operator, I want the private benchmark content to remain outside Git and move
...
70. As the operator, I want unmanaged state reported and pruned only by a separate
    export-backed operation, so that ordinary reconcile is non-destructive.
71. As the operator, I want the legacy engineering bank rebuilt through a verified
    shadow bank, so that old observation scopes are normalized without direct
    database edits.
72. As the operator, I want the historical candidate bank deleted in the approved
...
74. As the operator, I want content-bearing native audit and LLM trace storage disabled,
    so that personal and confidential prompts are not duplicated into
    operational tables.
75. As the operator, I want a content-free controller ledger retained for 90 days, so
...
78. As the operator, I want airlocked CLI agents to run in isolated OrbStack machines,
    so that hostile shell actions cannot reach host memory state, Keychain
    credentials, or the core Hindsight token.
79. As the operator, I want each machine's ordinary engineering bank to remain its one
...
83. As the operator, I want candidate reconciliation manifests bound through the final
...
84. As the operator, I want migration artifacts retained until a separate digest-bound
    retirement approval, so that live-bank closeout cannot silently remove
    rollback or provenance evidence.
85. As the operator, I want organization-specific model IDs, repository tags, and legacy
...
- `nisavid/agents` owns schemas, provider and model catalogs, bank-archetype
  fragments, overlay definitions, controller code, harness adapter policy,
  tests, skills, and documentation.
- A consuming configuration repository owns machine inventory, secret locators,
  service bindings, and disabled rendered artifacts. Rendering or installation
  never calls Hindsight mutation APIs, starts ambient memory hooks, downloads
  models, logs in providers, or prunes state.
...
- Every profile enables Hindsight's API-key tenant extension. Its distinct
  bearer token is stored only in the consuming machine's protected secret
  store and delivered to the fleet controller and matching profile process
  through their private process environments. CLI arguments carry only the
  environment-variable locator. The controller uses the value only in the
  backend authorization header. Plans, rendered configuration, process
  arguments, logs, harnesses, browsers, and child tools never receive or expose
  a data-plane token.
- The Hindsight control-plane HTTP API binds only to loopback and requires an
  independent machine-local access key on every endpoint. The reusable control
  plane is CLI-only: it exposes no browser proxy and does not render, log, or
  place that key in browser storage. The existing Hindsight Embed browser UI is
  a distinct profile data-plane surface and is not evidence of control-plane
  authentication. Browser access to the control plane remains unsupported until
  a server-side local-session bootstrap can authenticate the browser without
  disclosing the access key.
...
- Each `contextual_model_migrations` entry has a unique `source_id`, a
  ...
  this resolution is exactly `operator-profile` and
  `engineering-principles`; a private target must also be declared in
...
- `hindsight-memory session` asks the broker to mint a signed, opaque, one-use
  ...
  The launcher removes the handle variable before executing the harness, and
  neither the one-use envelope nor its handle may appear in a harness,
  tool-server, shell, hook, or other descendant process environment. Descendants
  receive only the exchanged, method-scoped session capability through the
  authenticated IPC contract.
...
- One consumer-supplied per-user service binding owns one fleet supervisor.
...
- A separate control-plane API health gate proves loopback-only binding,
  rejects missing and incorrect access keys and unauthenticated proxy paths,
  and confirms that responses cannot reveal a profile bearer token. It does not
  claim that the distinct Hindsight Embed browser UI is control-plane-authenticated.
...
- Catalog order records the operator's current preference hypothesis, not a quality
...
- Apple-silicon local models use MLX when available and OptiQ when compatible.
  ...
  encrypted into configuration source.
...
- Target selection is not a quality claim. A model becomes recommended or
  ...
  pass.
- Credentials, OAuth homes, model caches, and active revisions remain local to
  the consuming machine and are never copied into reusable source.
- Candidate state is never presented as active state. Every provider or model
  switch is explicit-plan-only and visibly degraded when a fallback is used.

### Credential policy

- Consuming configuration stores authentication modes and non-secret locators
  only. It stores no provider credential value.
- Stable API keys use a platform credential store. The fleet resolves them
...
- The observations mission synthesizes durable cross-domain operating rules,
  ...
  belief.
- The reflect mission identifies the bank as the operator's shared engineering memory
...
- The migration disposition map is exact: `operating-profile` is superseded by
  ...
  disposition in the private deployment catalog; `review-and-pr-playbook` and
  `project-context` have no emitted successor; and `engineering-principles` is
  new. Old models remain available but uninjected
...
- The reflect mission identifies the bank as the operator's private personal memory for
...
- The exact routine/deep caps are respectively: fact text 10,000/20,000 tokens,
  entities 32/64, chunks 16/32, source facts 32/64, injected mental models 3/3,
  and complete controller output 16,000/30,000 tokens. The 10,000/20,000 fact
  allowance is included inside—not added to—the complete controller cap.
  Assembly order is the advisory header and provenance, operator profile,
  transferable engineering principles, the one matched contextual model when
  present, ranked facts, entities, chunks, then source facts. When the aggregate
  cap is reached, the controller removes items from the tail of that order,
  lowest-ranked first within a type, and emits per-type omitted counts plus the
  final aggregate token count. It never partially emits an item or reorders
  equal-ranked items; ties use stable identity byte order.
...
- Deterministic deny rules run before the reviewer and apply the union of
...
- Consumer configuration renders inactive hooks and settings. Activation is a
  separate, reversible controller apply bound to an approved digest, unchanged
  active snapshot, and successful endpoint, bank-reference, capability, and
  loop-hygiene checks.
...
- Promotion compares only reports with the same private-dataset digest and
  deployment envelope. Every report and decision also binds the benchmark
  schema digest and the digest of the complete promotion policy, including the
  versioned material-regression and meaningful-gain threshold tables; a digest
  mismatch is a closed gate, not a new comparison. Effects are candidate minus baseline for Recall@20 and
  nDCG@10, and baseline minus candidate for latency, cost, peak memory, and model
  footprint, so positive always means improvement. Threshold comparisons use
  exact unrounded point effects with a fixed `1e-12` numerical tolerance; report
  formatting never changes a decision. Bootstrap intervals use the recorded
  seed, sample count, confidence, and identical case-index resamples for every
  metric. They are reported evidence, not a substitute for the declared
  material-regression and meaningful-gain thresholds. A must-not-return match at
  any returned rank is a leakage failure; it is never limited to the Recall@20
  or nDCG@10 windows. Policy or leakage failure is independently disqualifying
  even when the aggregate retrieval gate is otherwise green.
- Candidate reports are canonically ordered by deployment envelope and
  candidate identity before frontier and promotion decisions. Re-evaluating
  identical report bodies, schema, policy, thresholds, and bootstrap parameters
...
- The migration-artifact directory stores the encrypted canonical-source bank
  ...
  available until the operator separately approves a digest-bound archive-retirement
...
- Read-only discovery requires one server-backed opaque monotonic generation
  spanning the complete read. The generation must be unchanged before and after
  discovery; ordinary before/after state equality is only an additional drift
  check. If the adapter cannot provide that generation, or if it changes, the
  controller fails closed, writes no discovery result, and performs no live
  mutation.
- The generation is the source-bank snapshot generation: the server advances it
  on every committed mutation that can change the source or candidate bank,
  their invalidation archive, hooks, schedules, operation state, or retain
  watermark. The two bank inventories and their related state are read from one
  server snapshot between generation reads; a client-computed digest, process
  counter, timestamp, or independently sampled endpoint value does not satisfy
  this contract.
...
- The source discovery generation is bound into the source manifest and shadow
  plan as the atomic consistency proof for source reads. It is not a shadow
  write receipt or target generation. Any later source read must remain within
  that same server-backed generation; a missing or unequal generation
  invalidates the source evidence and restarts discovery. Retain watermarks and
  ordinary state equality remain secondary drift checks.
- Each approved shadow write returns a separate target receipt that binds the
  mutation-plan digest, canonical shadow-bank reference, projection-batch
  digest, and server-backed target generations immediately before and after the
  write. Each receipt's post-write generation must equal the next receipt's
  pre-write generation. The complete receipt chain is bound into the resulting
  shadow-content digest and cutover plan; missing, unequal, or unchained target
  generations invalidate the shadow evidence.
...
- The authoritative full-schema command pair is the version-pinned
  `hindsight-admin backup` and `hindsight-admin restore`; direct `pg_dump`,
  `pg_restore`, table copy, and HTTP bank import are not substitutes. The
  adapter records the exact redacted argv and tool-version digests, requires a
  successful exit, encrypts and digests the backup before publication, then
  runs `hindsight-admin restore` only against a disposable compatibility
  database. Restore acceptance requires schema/version compatibility, the exact
  bank inventory, invalidated-memory count and item digests, and a fresh
  cold-cache read. Only that accepted artifact may be bound as the canonical-ID
  rollback bundle; live restore remains a separately approved rollback action.
...
- Authentication tests prove tenant-token enforcement on HTTP and MCP, no raw
  ...
  bank expansion.
- Control-plane API tests prove loopback-only binding, independent access-key
  enforcement on every control endpoint, rejection of missing and incorrect
  keys and unauthenticated proxies, token-free rendered files and logs, and the
  absence of any browser-auth bootstrap that would disclose the access key.
...
- Model adapter tests verify protocol request and response shape,
  tokenizer-derived scoring token IDs, normalized affirmative probability,
  stable ordering, concurrency bounds, health/version metadata, explicit
  fallback, and no automatic switching. Provider-specific payload gates belong
  to the consuming deployment catalog.
...
- Fleet lifecycle tests cover one per-user service binding, multiple profiles,
  persisted slots, endpoint collisions, process ownership,
  credential-generation drift, sidecar health, bounded waits, token-free logs,
  and the existing five-command interface. Consumer repositories test their
  concrete service manifest and rendering adapter separately.
