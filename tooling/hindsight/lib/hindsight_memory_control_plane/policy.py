...
ENGINEERING_REFLECT_MISSION = (
    "Treat this bank as the operator's shared engineering memory across approved "
...
PERSONAL_REFLECT_MISSION = (
    "Treat this bank as the operator's private personal memory for explicitly personal "
...
) | frozenset(PROJECTION_DENY_CLASSES)
PROJECTION_POLICY = deep_freeze({
    "minimal": True,
    "idempotent": True,
    "provenance_linked": True,
    "independently_deletable": True,
    "deny_policy": "source-target-intersection",
    "deny_classes": PROJECTION_DENY_CLASSES,
    "reviewer_bounds": {
...
def _string_sequence(value: Any, label: str) -> list[str]:
    if not isinstance(value, list) or any(
        not isinstance(member, str) for member in value
    ):
...
class BankPolicy:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "enable_auto_consolidation": self.enable_auto_consolidation,
...
class PolicyArtifact:
    ...
    contextual_models: tuple[ModelPolicy, ...] = field(repr=False)
    repository_tags: frozenset[str] = field(repr=False)
    workflow_tags: frozenset[str] = field(repr=False)
...
class RouteDecision:
    ...
    contextual_model_id: str | None = field(repr=False)
...
def resolve_policy(
    ...
    *,
    ...
    for raw_bank in raw_banks:
        ...
        if (
            not isinstance(raw_bank["authority"], str)
            or raw_bank["authority"]
            not in {"authoritative", "replica", "none"}
        ):
            raise PolicyError(f"bank {bank_id} has invalid authority")
        if not isinstance(raw_bank["kind"], str):
            raise PolicyError(f"bank {bank_id} has invalid kind")
    ...
    writable_engineering = [
        bank
        for bank in banks
        if bank.kind == "engineering" and bank.writable
    ]
    if engineering_enabled and (
        len(writable_engineering) != 1
        or writable_engineering[0].authority != "authoritative"
    ):
        raise PolicyError(
            "engineering memory requires exactly one authoritative write bank"
        )
    if not engineering_enabled and writable_engineering:
        raise PolicyError(
            "engineering-disabled policy cannot declare writable engineering banks"
        )
    ...
    for raw in raw_workspaces:
        ...
        mapped_bank_id = _identifier(
            raw["bank_id"], "workspace mapped bank"
        )
    ...
    for bank_id, values in raw_companions.items():
        if not isinstance(bank_id, str):
            raise PolicyError("allowed companion bank IDs must be strings")
        try:
            values = _string_sequence(
                values, f"allowed companions for {bank_id}"
            )
        except PolicyError as error:
            raise PolicyError(
                f"allowed companions for {bank_id} must reference declared banks"
            ) from error
        if any(value not in seen_banks for value in values):
            raise PolicyError(
                "allowed companions for "
                f"{bank_id} must reference declared banks"
            )
        if any(
            next(bank.authority for bank in banks if bank.id == value)
            == "none"
            for value in values
        ):
            raise PolicyError("allowed companions must have memory authority")
    ...
    if not engineering_enabled and any(
        (
            next(bank.kind for bank in banks if bank.id == bank_id)
            == "engineering"
            and values
        )
        or any(
            next(bank.kind for bank in banks if bank.id == value)
            == "engineering"
            for value in values
        )
        for bank_id, values in companions.items()
    ):
        raise PolicyError(
            "engineering-disabled policy cannot declare engineering routes"
        )
    ...
    if engineering_enabled and default_kind == "personal":
        raise PolicyError(
    ...
    if (
        ...
        or any(
            ...
            or target not in repository_tags
    ...
    for values, prefix, label, allow_empty in (
        (repository_tags, "repo:", "repository tags", False),
        (workflow_tags, "workflow:", "workflow tags", True),
    ):
        if (
    ...
    public_model_ids = {
        model.id for bank in banks for model in bank.models
    }
    ...
    for raw_model in raw_contextual:
        ...
        if model_id in public_model_ids:
            raise PolicyError(
                "contextual model IDs must not overlap public bank model IDs"
            )
        if not isinstance(selector, str):
            raise PolicyError(
                "contextual model selector must be a string"
            )
    ...
    for migration in migrations:
        ...
        if not isinstance(disposition, str):
            raise PolicyError(
                "contextual model migration disposition must be a string"
            )
        expected = (
        ...
        if disposition != "retire":
            target_id = _identifier(target_id, "migration target id")
        ...
        if (
            disposition != "retire"
            and target_id not in model_ids | public_targets
        ):
    ...
    def public_strings(value: Any) -> Iterable[str]:
        if isinstance(value, str):
            ...
            for key, nested in value.items():
                if isinstance(key, str):
                    yield key
                yield from public_strings(nested)
    ...
    leaked = sorted(
        ...
        if any(literal in emitted for emitted in public_strings(public_body))
    )
    if leaked:
        raise PolicyError("public policy body contains a private catalog literal")
...
def validate_tags(
    ...
    supplied = tuple(tags)
    if any(not isinstance(tag, str) for tag in supplied):
        raise PolicyError("tags must be strings")
    ...
    for tag in supplied:
        if tag not in allowed:
            raise PolicyError("unknown tag outside the closed vocabulary")
    return supplied
...
def validate_durable_policy_input(
    ...
    if any(not isinstance(input_class, str) for input_class in supplied):
        raise PolicyError("durable policy input classes must be strings")
...
def _validate_session_home(
    ...
    *,
    ...
    bank = policy.bank(home)
    kind = bank.kind
    if kind == "airlock":
        raise PolicyError("ordinary sessions cannot route into an isolated airlock")
    if kind == "engineering" and not policy.engineering_enabled:
        raise PolicyError("engineering memory is disabled")
    if kind == "personal" and not personal_session:
        raise PolicyError(
            "a personal home bank requires an explicitly personal session"
        )
    if not bank.writable or bank.authority != "authoritative":
        raise PolicyError("a session home bank must be writable authoritative")
...
def resolve_session_route(
    ...
    *,
    ...
    if explicit_home_bank is not None:
        explicit_home_bank = _identifier(
            explicit_home_bank, "explicit home bank"
        )
        if explicit_home_bank not in bank_ids:
            raise PolicyError("explicit home bank is not declared")
        home = explicit_home_bank
    else:
        matched_workspaces = tuple(matched_workspaces)
        if any(not isinstance(value, str) for value in matched_workspaces):
            raise PolicyError("matched workspace selectors must be strings")
        matches = [
            value
    ...
    if personal_session and policy.bank(home).kind != "personal":
        raise PolicyError(
            "an explicitly personal route requires a personal home bank"
        )
    ...
    if any(not isinstance(value, str) for value in requested):
        raise PolicyError("requested companion banks must be strings")
    ...
    includes_personal = policy.bank(home).kind == "personal" or any(
        policy.bank(value).kind == "personal" for value in requested
    )
    if includes_personal and not personal_session:
        raise PolicyError(
            "a personal companion route requires an explicitly personal session"
        )
    if personal_session and any(
        policy.bank(value).kind != "personal" for value in requested
    ):
        raise PolicyError(
            "an explicitly personal route requires personal companion banks"
        )
    if personal_session and not includes_personal:
        raise PolicyError(
            "an explicitly personal session requires a personal route"
        )
    if not policy.engineering_enabled and any(
        policy.bank(value).kind == "engineering" for value in requested
    ):
        raise PolicyError("engineering memory is disabled")
    workflows = tuple(workflow_selectors)
    repositories = tuple(repository_selectors)
    if any(not isinstance(value, str) for value in workflows):
        raise PolicyError("workflow selectors must be strings")
    if any(not isinstance(value, str) for value in repositories):
        raise PolicyError("repository selectors must be strings")
    contextual = None if personal_session else _contextual_selection(
        policy, workflows, repositories
    )
