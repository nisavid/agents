...
typeset -a PG_NAMES=()
typeset -A PG_DATA_DIRS=()
typeset -a PARTIAL_PG_NAMES=()
typeset -A PARTIAL_PG_DATA_DIRS=()
typeset CURRENT_PHASE=bootstrap
typeset CLEANED_UP=0
typeset CLEANUP_RUNNING=0
typeset PENDING_SIGNAL_STATUS=0

api_process_identity() {
  emulate -L zsh
  local pid=$1 output
  local -a fields
  output=$(/bin/ps -p "$pid" -o ppid= -o lstart= 2>/dev/null) || return 1
  fields=(${=output})
  (( ${#fields} == 6 )) || return 1
  print -r -- "${(j: :)fields}"
}

api_process_is_zombie() {
  emulate -L zsh
  local pid=$1 state
  state=$(/bin/ps -p "$pid" -o state= 2>/dev/null) || return 1
  state="${state//[[:space:]]/}"
  [[ "$state" == Z* ]]
}

api_process_absent_and_reap() {
  emulate -L zsh
  local pid=$1
  kill -0 "$pid" >/dev/null 2>&1 && return 1
  wait "$pid" >/dev/null 2>&1 || true
  unset "API_IDENTITIES[$pid]"
  return 0
}
...
api_identity_matches() {
  ...
  expected=${API_IDENTITIES[$pid]-}
  [[ -n "$expected" ]] || return 1
  current=$(api_process_identity "$pid") || return 1
  [[ "$current" == "$expected" && "${current%% *}" == "$$" ]]
}

api_identity_was_replaced() {
  emulate -L zsh
  local pid=$1 expected current
  expected=${API_IDENTITIES[$pid]-}
  [[ -n "$expected" ]] || return 1
...
terminate_api_pid() {
  ...
  if ! api_identity_matches "$pid"; then
    api_process_absent_and_reap "$pid" && return 0
    print -u2 -- "refusing to signal Hindsight API with changed process identity: ${pid}"
    return 1
  fi
  kill -TERM "$pid" >/dev/null 2>&1 || true
  for attempt in {1..50}; do
    if api_process_is_zombie "$pid"; then
      wait "$pid" >/dev/null 2>&1 || true
      unset "API_IDENTITIES[$pid]"
      return 0
    fi
    ...
    if ! api_identity_matches "$pid"; then
      api_process_absent_and_reap "$pid" && return 0
      if api_identity_was_replaced "$pid"; then
        unset "API_IDENTITIES[$pid]"
        return 0
      fi
      print -u2 -- "refusing to continue signaling Hindsight API with changed process identity: ${pid}"
      return 1
    fi
    sleep 0.1
  done
  if ! api_identity_matches "$pid"; then
    ...
    print -u2 -- "refusing to kill Hindsight API with changed process identity: ${pid}"
    return 1
  fi
  ...
  for attempt in {1..50}; do
    ...
    if ! api_identity_matches "$pid"; then
      ...
      print -u2 -- "Hindsight API identity changed after KILL: ${pid}"
      return 1
    fi
    sleep 0.1
  done
  print -u2 -- "disposable Hindsight API did not terminate: ${pid}"
  return 1
}

terminate_started_api_pid() {
  emulate -L zsh
  unsetopt ERR_EXIT
  local pid=$1
  ...
  if ! kill -0 "$pid" >/dev/null 2>&1; then
    wait "$pid" >/dev/null 2>&1 || true
    API_PIDS=(${API_PIDS:#$pid})
    unset "API_IDENTITIES[$pid]"
    return 0
  fi
  for attempt in {1..5}; do
    before=$(api_process_identity "$pid") || {
      sleep 0.02
      continue
    }
    [[ "${before%% *}" == "$$" ]] || break
    sleep 0.02
    after=$(api_process_identity "$pid") || continue
    if [[ "$after" == "$before" && "${after%% *}" == "$$" ]]; then
      API_IDENTITIES[$pid]="$after"
      terminate_api_pid "$pid" || return 1
      API_PIDS=(${API_PIDS:#$pid})
      return 0
    fi
  done
  if api_process_absent_and_reap "$pid"; then
    API_PIDS=(${API_PIDS:#$pid})
    return 0
  fi
  print -u2 -- "refusing to signal newly started Hindsight API without a stable direct-child identity: ${pid}"
  return 1
}
...
$OPENSSL rand -hex 48 >"$EXPORT_PASSPHRASE_FILE"
print -r -- "header = \"Authorization: Bearer $API_KEY\"" >"$CURL_AUTH_CONFIG"
chmod 600 "$CURL_AUTH_CONFIG"

finish_cleanup() {
  emulate -L zsh
  local result="$1" pending_signal="$PENDING_SIGNAL_STATUS"
  CLEANUP_RUNNING=0
...
cleanup() {
  ...
  CLEANUP_RUNNING=1
  local pid name data_dir
  integer drop_failures=0 verification_failed=0
  for pid in $API_PIDS; do
    terminate_api_pid "$pid" || verification_failed=1
  done
  for name in $PG_NAMES $PARTIAL_PG_NAMES; do
    if [[ -n "${PG_DATA_DIRS[$name]-}" ]]; then
      data_dir=$PG_DATA_DIRS[$name]
    else
      data_dir=$PARTIAL_PG_DATA_DIRS[$name]
    fi
    HOME="$SMOKE_HOME" PG0_NAME="$name" PG0_DATA_DIR="$data_dir" PG0_RUN_ID="$SMOKE_RUN_ID" \
      PG0_SMOKE_ROOT="$SMOKE_ROOT" \
      PG0_REGISTRATION="$SMOKE_HOME/.pg0/instances/$name" \
      "$HINDSIGHT_PYTHON" -c '
import json
import os
...
run_id = os.environ["PG0_RUN_ID"]
expected_data_dir = Path(os.environ["PG0_DATA_DIR"]).resolve()
smoke_root = Path(os.environ["PG0_SMOKE_ROOT"]).resolve()
registration = Path(os.environ["PG0_REGISTRATION"])
expected_identity = next(
    (
        "hindsight_{}_{}".format(role, run_id)
        for role in ("source", "import", "restore")
        if name.startswith(f"hindsight-smoke-{run_id}-{role}-")
    ),
    None,
)
try:
    expected_data_dir.relative_to(smoke_root)
except ValueError:
    raise SystemExit("refusing to drop a PostgreSQL instance outside the smoke root")
...
if (
    ...
    or smoke_metadata.st_uid != os.geteuid()
    or stat.S_IMODE(smoke_metadata.st_mode) & 0o022
    or not stat.S_ISDIR(data_metadata.st_mode)
    or stat.S_ISLNK(data_metadata.st_mode)
    or data_metadata.st_uid != os.geteuid()
...
try:
    ...
    raise SystemExit(3)
registration_parent = Path.home() / ".pg0" / "instances"
for ancestor in (Path.home(), Path.home() / ".pg0", registration_parent):
    try:
    ...
    if (
        ...
        raise SystemExit(3)
if (
    registration != registration_parent / name
    or not stat.S_ISDIR(registration_metadata.st_mode)
    or stat.S_ISLNK(registration_metadata.st_mode)
    or registration_metadata.st_uid != os.geteuid()
...
try:
    ...
    if len(raw) > 65536 or os.read(descriptor, 1):
        raise ValueError("oversized manifest")
    current = manifest.lstat()
    if (current.st_dev, current.st_ino) != (opened.st_dev, opened.st_ino):
        raise ValueError("changed manifest")
    registered = json.loads(raw.decode("utf-8"))
except Exception:
    raise SystemExit(3)
finally:
    if descriptor is not None:
        os.close(descriptor)
if (
    registered.get("data_dir") is None
    or Path(registered["data_dir"]).resolve() != expected_data_dir
    or registered.get("username") != expected_identity
    or registered.get("database") != expected_identity
):
    raise SystemExit(3)
matches = [instance for instance in pg0.list_instances() if instance.name == name]
if not matches:
    raise SystemExit(0)
if (
    len(matches) != 1
    or expected_identity is None
    or matches[0].data_dir is None
    or Path(matches[0].data_dir).resolve() != expected_data_dir
    or matches[0].username != expected_identity
    or matches[0].database != expected_identity
):
    raise SystemExit("refusing to drop a PostgreSQL instance not owned by this smoke run")
pg0.Pg0(name=name).drop(force=True)
remaining = [instance for instance in pg0.list_instances() if instance.name == name]
if remaining:
    if any(instance.running for instance in remaining):
        raise SystemExit("run-owned PostgreSQL instance remains active after drop")
    ...
    try:
        ...
        current_registration = os.fstat(registration_descriptor)
        ...
        if (
            (current_registration.st_dev, current_registration.st_ino)
            != (registration_metadata.st_dev, registration_metadata.st_ino)
            or (current_manifest.st_dev, current_manifest.st_ino)
            != (manifest_metadata.st_dev, manifest_metadata.st_ino)
        ):
            raise SystemExit(3)
        os.unlink("instance.json", dir_fd=registration_descriptor)
...
if any(instance.name == name for instance in pg0.list_instances()):
    raise SystemExit("run-owned PostgreSQL instance remains after cleanup")
' >/dev/null
    local drop_rc=$?
    if (( drop_rc != 0 )); then
      (( drop_failures += 1 ))
      (( drop_rc == 3 )) && verification_failed=1
    fi
  done
  local remaining_instances
  remaining_instances=$(
    HOME="$SMOKE_HOME" SMOKE_PG_NAMES="${(j:,:)PG_NAMES},${(j:,:)PARTIAL_PG_NAMES}" "$HINDSIGHT_PYTHON" -c '
import os
import pg0
...
'
  ...
  if (( verification_failed )) || [[ "$remaining_instances" != 0 ]]; then
    print -u2 -- "disposable smoke cleanup retained ${SMOKE_ROOT}: pg0 cleanup could not verify all run-owned instances absent"
    finish_cleanup 1
    return $?
  fi
  if (( drop_failures )); then
    print -u2 -- "disposable smoke cleanup observed ${drop_failures} pg0 drop failure(s), but verified all run-owned instances absent"
  fi
  case "$SMOKE_ROOT" in
...
request_signal_shutdown() {
  ...
  (( PENDING_SIGNAL_STATUS )) || PENDING_SIGNAL_STATUS="$signal_status"
  (( CLEANUP_RUNNING )) && return 0
  cleanup || true
  local pending_signal="$PENDING_SIGNAL_STATUS"
  trap - EXIT
  exit "$pending_signal"
}

TRAPINT() {
  request_signal_shutdown 130
}

TRAPTERM() {
  request_signal_shutdown 143
}
...
start_postgres() {
  ...
  local role=$4
  local password=$5
  HOME="$SMOKE_HOME" PG0_NAME="$name" PG0_PORT="$port" PG0_DATA_DIR="$data_dir" \
...
start_postgres_on_free_port() {
  ...
  local password=$3
  local output_file=$4
  local attempt port name attempt_data_dir
  for attempt in {1..10}; do
    port=$(free_port)
    name="hindsight-smoke-${SMOKE_RUN_ID}-${role}-${port}"
    attempt_data_dir="${data_dir}.attempt-${attempt}"
    if start_postgres "$name" "$port" "$attempt_data_dir" "$role" "$password" \
      >"$output_file" 2>"${output_file}.log"; then
      PG_NAMES+=("$name")
      PG_DATA_DIRS[$name]=$attempt_data_dir
      STARTED_PG_PORT=$port
      STARTED_PG_NAME=$name
      return 0
    fi
    # A failed start may still have published a pg0 registration or child.
    # Track it separately for best-effort cleanup without treating it as a
    # successfully provisioned smoke dependency.
    PARTIAL_PG_NAMES+=("$name")
    PARTIAL_PG_DATA_DIRS[$name]=$attempt_data_dir
  done
  sed -n '1,120p' "${output_file}.log" >&2
...
start_api() {
  ...
  identity=$(api_process_identity "$STARTED_API_PID") || {
    print -u2 -- "could not capture disposable Hindsight API process identity"
    terminate_started_api_pid "$STARTED_API_PID" || true
    return 1
  }
  [[ "${identity%% *}" == "$$" ]] || {
    print -u2 -- "disposable Hindsight API is not a direct child"
    terminate_started_api_pid "$STARTED_API_PID" || true
    return 1
  }
  API_IDENTITIES[$STARTED_API_PID]=$identity
}
...
start_api_on_free_port() {
  ...
  for attempt in {1..10}; do
    port=$(free_port)
    if ! start_api "$database_url" "$role" "$port" "$log_file"; then
      continue
    fi
    pid=$STARTED_API_PID
...
authenticated_get() {
  ...
  local path=$2
  local output=$3
  "$CURL" --noproxy '*' --silent --show-error --fail --max-time 30 \
...
typeset -r RESTORE_TRANSFER_COUNTS=$(bank_transfer_counts "$RESTORE_DB_URL" "$SOURCE_BANK")
[[ "$RESTORE_FINGERPRINT" == "$SOURCE_FINGERPRINT" ]]
[[ "$RESTORE_TRANSFER_COUNTS" == "$SOURCE_TRANSFER_COUNTS" ]]
...
typeset -r REMAINING_SMOKE_INSTANCES=$(
  HOME="$SMOKE_HOME" SMOKE_PG_NAMES="${(j:,:)PG_NAMES}" "$HINDSIGHT_PYTHON" -c '
import os
import pg0
