        ...
        async def prepare():
            ...
            try:
                ...
                await connection.execute(
                    ...
                    "'engineering', 'Alice', '{}'::jsonb, "
                    "'2026-08-12T00:00:00Z'::timestamptz, "
                    "'2026-08-12T00:00:00Z'::timestamptz, 1)"
                )
            finally:
                await connection.close()
...
"""
        ...
        if schema_version == 4:
            ...
            await connection.execute(
                ...
                SET status = 'failed', retry_count = 3,
                    error_message = 'unclassified retain failure',
                    worker_id = $2,
                    claimed_at = '2026-08-14T01:49:50Z',
                    completed_at = '2026-08-14T02:49:32Z',
                    updated_at = '2026-08-14T02:49:32Z'
                WHERE operation_id = $1::uuid
                """,
                failed_id,
                worker_id,
            ...
            for operation_id, retry_count in zip(
                remaining_retain_ids,
                (0, 3, 2, 2),
                strict=True,
            ):
            ...
            await connection.execute(
                ...
                SET status = 'failed', retry_count = 3,
                    error_message = '',
                    worker_id = $2,
                    claimed_at = '2026-08-13T08:00:00Z',
                    completed_at = '2026-08-13T15:20:00Z',
    ...
    async def _post_abort_v10_case(self, connection):
        ...
        completed_ids = [item["operation_id"] for item in selected[:2]]
        failed_ids = [item["operation_id"] for item in selected[2:24]]
        pending_ids = [item["operation_id"] for item in selected[24:40]]
        processing_ids = [selected[40]["operation_id"]]
        worker_id = exact_drain_worker_id(reference["plan_digest"])
        await connection.execute(
            """
            UPDATE public.async_operations
            SET status = 'completed', retry_count = 3,
                worker_id = $2,
                claimed_at = '2026-08-15T19:15:00Z',
                completed_at = '2026-08-15T20:14:00Z',
        ...
        await connection.execute(
            ...
            UPDATE public.async_operations
            SET status = 'failed', retry_count = 3,
                error_message = 'unclassified operation failure',
                worker_id = $2,
                claimed_at = '2026-08-15T19:15:00Z',
        ...
        await connection.execute(
            ...
            UPDATE public.async_operations
            SET retry_count = 3,
                next_retry_at = '2026-08-15T20:16:00Z',
                error_message = 'provider transport timeout',
                worker_id = $2,
                claimed_at = '2026-08-15T19:15:00Z',
                updated_at = '2026-08-15T20:15:00Z'
        ...
        await connection.execute(
            """
            UPDATE public.async_operations
            SET status = 'processing', retry_count = 3,
                error_message = 'provider transport timeout',
                worker_id = $2,
                claimed_at = '2026-08-15T19:15:00Z',
                updated_at = '2026-08-15T20:15:00Z'
        ...
        plan = create_post_abort_recovery_plan(
            ...
            candidate_release={
                "source_commit": "4" * 40,
                "version": "2026.08.15+4444444.operation-recovery.39",
                "release_digest": "5" * 64,
            },
            rollback_backup=rollback,
            rollback_encryption=recovery_fixtures.rollback_encryption(),
            rollback_backup_path="/private/tmp/post-abort-v10-backup.age",
            rollback_bundle_path="/private/tmp/post-abort-v10-bundle.age",
            authorization_receipt_path=(
                "/private/tmp/post-abort-v10-authorization.json"
            ),
            application_receipt_path=(
                "/private/tmp/post-abort-v10-application.json"
            ),
            verification_receipt_path=(
                "/private/tmp/post-abort-v10-verification.json"
            ),
            rollback_receipt_path=(
                "/private/tmp/post-abort-v10-rollback.json"
            ),
        ...
        return (
            plan,
            cohort_ids,
            [*failed_ids, *pending_ids, *processing_ids],
            completed_ids,
            unexpected_id,
        )
    ...
    def test_post_abort_v5_resets_exact_five_and_preserves_forty_three(self):
        async def exercise():
            connection = await self._connect()
            try:
                plan, cohort_ids, selected_ids, unexpected_id = (
                    await self._post_abort_case(connection, schema_version=5)
                )
                before = {
                    row["operation_id"]: row
                    for row in await read_safe_operation_rows(
                ...
                for operation_id in selected_ids:
                    ...
                    if before[operation_id]["status"] == "failed":
                        self.assertEqual(after[operation_id]["retry_count"], 0)
                        self.assertIsNone(after[operation_id]["completed_at"])
                        self.assertEqual(after[operation_id]["error_category"], "none")
    ...
    def test_post_abort_v6_resets_exact_four_and_preserves_forty_four(self):
        async def exercise():
            connection = await self._connect()
            try:
                plan, cohort_ids, selected_ids, unexpected_id = (
                    await self._post_abort_case(connection, schema_version=6)
                )
                before = {
                    row["operation_id"]: row
                    for row in await read_safe_operation_rows(
                ...
                for operation_id in selected_ids:
                    ...
                    if before[operation_id]["status"] == "failed":
                        ...
                        self.assertIsNone(after[operation_id]["error_digest"])
                preserved_ids = set(cohort_ids) - set(selected_ids)
                self.assertEqual(len(preserved_ids), 44)
                for operation_id in preserved_ids:
                    self.assertEqual(
                        live_row_digest(after[operation_id]),
                        live_row_digest(before[operation_id]),
    ...
    def test_post_abort_v8_resets_exact_two_and_preserves_forty_six(self):
        async def exercise():
            connection = await self._connect()
            try:
                plan, cohort_ids, selected_ids, unexpected_id = (
                    await self._post_abort_case(connection, schema_version=8)
                )
                before = {
                    row["operation_id"]: row
                    for row in await read_safe_operation_rows(
                ...
                self.assertEqual(
                    {
                        row["status"]: (row["retry_count"], row["error_category"])
                        for operation_id in selected_ids
                        if (row := before[operation_id])
                    },
                    {
                        "failed": (3, "unknown"),
                        "processing": (1, "provider_transport"),
                    },
                )
                ...
                for operation_id in selected_ids:
                    prior = before[operation_id]
                    current = after[operation_id]
                    self.assertEqual(current["status"], "pending")
                    self.assertFalse(current["worker_id_present"])
                    self.assertIsNone(current["claimed_at"])
                    self.assertEqual(
                        current["result_metadata_digest"],
                        prior["result_metadata_digest"],
                    )
                    if prior["status"] == "failed":
                        self.assertEqual(current["retry_count"], 0)
                        self.assertEqual(current["error_category"], "none")
                        self.assertIsNone(current["error_digest"])
                    else:
                        ...
                        self.assertEqual(
                            current["error_digest"],
                            prior["error_digest"],
                        )
                preserved_ids = set(cohort_ids) - set(selected_ids)
                self.assertEqual(len(preserved_ids), 46)
                for operation_id in preserved_ids:
                    self.assertEqual(
                        live_row_digest(after[operation_id]),
                        live_row_digest(before[operation_id]),
    ...
    def test_post_abort_v10_applies_observed_shape_and_preserves_checkpoints(self):
        async def exercise():
            connection = await self._connect()
            try:
                (
                ...
                self.assertEqual(len(selected_ids), 39)
                ...
                for operation_id in selected_ids:
                    ...
                    self.assertIsNone(after[operation_id]["claimed_at"])
                    self.assertEqual(
                        after[operation_id]["result_metadata_digest"],
                        before[operation_id]["result_metadata_digest"],
                    )
                    if expected_status[operation_id] == "failed":
                        self.assertEqual(after[operation_id]["retry_count"], 0)
                        self.assertEqual(
                ...
                for operation_id in set(cohort_ids) - set(selected_ids):
                    self.assertEqual(
                        live_row_digest(after[operation_id]),
                        live_row_digest(before[operation_id]),
                    )
    ...
    def test_exact_drain_terminal_write_uses_reserved_pool_connection(self):
        async def exercise():
            ...
            try:
                ...
                adapter = self._exact_drain_adapter(plan)
                progress_path = Path(self.temporary.name) / "terminal-progress.json"
                ...
                operation_id = str(claimed[0]["operation_id"])
                checkpoint_sentinel = "not-in-progress-evidence"
                await connection.execute(
                    """
                    UPDATE public.async_operations
                    SET result_metadata = $2::jsonb
                    WHERE operation_id = $1::uuid
                    """,
                    operation_id,
                    json.dumps(
                ...
                self.assertEqual(
                    task["checkpoint"]["committed_document_count"],
                    1,
                )
                ...
                self.assertNotIn(checkpoint_sentinel, serialized_progress)
                self.assertNotIn("TimeoutError", serialized_progress)
                self.assertNotIn("result_metadata", serialized_progress)
            finally:
            ...
            try:
                ...
                class Recorder:
                    def task_stage(self, *arguments, **keywords):
                        outcomes.append(("stage", arguments, keywords))
                ...
                selected_outcomes = [
                    ...
                    if item[1] == (selected_id,)
                ]
                kind, arguments, evidence = selected_outcomes[-1]
                self.assertEqual(kind, "outcome")
                self.assertEqual(arguments, (selected_id,))
    ...
    def test_exact_drain_claim_release_preserves_pending_transport_state(self):
